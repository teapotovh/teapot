...
func ObservabilityFlagSet(serviceName string) (*flag.FlagSet, func() ObservabilityConfig) {
	...
	tracingEndpoint := fs.String("observability-tracing-endpoint", "0.0.0.0:4137", "the grpc endpoint for OTLP opentelemetry trace collection")
	tracingServiceName := fs.String("observability-tracing-service-name", serviceName, "the opentelemetry service name for traces produced by this application")
	tracingConnectTimeout := fs.Duration("observability-tracing-connect-timeout", time.Minute, "the timeout for the initial connection to the opentelemetry-collector")
