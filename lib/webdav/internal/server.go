...
type WithMkcalendar interface {
	Mkcalendar(r *http.Request) error
}
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if h.Backend == nil {
		...
		case "MKCALENDAR":
			if withMkcalendar, ok := h.Backend.(WithMkcalendar); ok {
				err = withMkcalendar.Mkcalendar(r)
				if err == nil {
					w.WriteHeader(http.StatusCreated)
				}
			} else {
...
type PropFindFunc func(raw *RawXMLValue) (any, error)

func PropFindValue(value any) PropFindFunc {
	return func(raw *RawXMLValue) (any, error) {
		return value, nil
	}
}
