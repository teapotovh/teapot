...
func checkConditionalMatches(fi *FileInfo, ifMatch, ifNoneMatch ConditionalMatch) error {
	...
	if ifMatch.IsSet() {
		if ok, err := ifMatch.MatchETag(etag); err != nil {
			...
			return NewHTTPError(http.StatusPreconditionFailed, errors.New("If-Match condition failed"))
		}
	}
	...
	if ifNoneMatch.IsSet() {
		if ok, err := ifNoneMatch.MatchETag(etag); err != nil {
			...
			return NewHTTPError(http.StatusPreconditionFailed, errors.New("If-None-Match condition failed"))
		}
	}
...
func (fs LocalFileSystem) Create(
	ctx context.Context,
	name string,
	body io.ReadCloser,
	opts *CreateOptions,
) (fi *FileInfo, created bool, err error) {
	p, err := fs.localPath(name)
	if err != nil {
