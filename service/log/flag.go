...
func LogFlagSet() (*flag.FlagSet, func() LogConfig) {
	...
	maxLogLinesBeforeFlush := fs.Uint32("log-max-lines-before-flush", 100, "maximum number of lines buffered before flushing to disk")
	rotateInterval := fs.Duration("log-rotate-interval", 1*time.Second, "the interval at which log files are rotated")
	maxFileSizeBeforeRotate := fs.Uint64("log-max-file-size-before-rotate", 10*1<<30, "maximum amount of bytes before a log is rotated")
